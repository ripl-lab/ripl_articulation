...
def solve_articulation_from_points(poses_a  : np.ndarray,
                                   points_b : np.ndarray,
                                   prior_xi : np.ndarray=None,
                                   prior_theta : float=0.0,
                                   xi_noise_model=gtsam.noiseModel.Diagonal.Sigmas(np.ones(6)),
    ...
    point_noise_model = gtsam.noiseModel.Diagonal.Sigmas(pose_noise_model.sigmas()[:3])

    for idx, (w_T_a, w_P_b) in enumerate(zip(poses_a, points_b)):
        thetas.append(sym_theta:=gtsam.symbol('T', idx))
        ...
        w_T_a_gtsam = gtsam.Pose3(np.require(w_T_a, requirements='F')) if w_T_a.ndim == 2 else gtsam.Pose3.Expmap(w_T_a)
        w_P_b_gtsam = np.require(w_P_b, requirements='F')

        graph.addPriorPose3(sym_a, w_T_a_gtsam, pose_noise_model)
        graph.addPriorPoint3(sym_b, w_P_b_gtsam, point_noise_model)

        graph.add(RelativePointFactor(sym_a, sym_b, sym_theta, sym_xi, point_noise_model))

        initial_values.insert(sym_a, w_T_a_gtsam)
        initial_values.insert(sym_b, w_P_b_gtsam)
        initial_values.insert(sym_theta, prior_theta)
